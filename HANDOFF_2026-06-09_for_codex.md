> ⚠️ **이 문서는 구버전입니다. 최신 인수인계는 `HANDOFF_2026-06-10.md` 를 보세요.**
> (2026-06-10 세션에서 점수 엔진 확장·심리 축·조건주문 기준가·모바일 페이저/하단탭·
>  갱신 스케줄 변경 등 다수 변경됨.)
...
배포 주소(현재 운영): https://sweetproduct.pages.dev
GitHub 저장소: https://github.com/jinnn99/sp-profit-plan
레거시/정적 미리보기: https://sweetproduct.netlify.app
...
- **배포 대상은 `mobile_app/` 폴더.** 현재 운영은 Cloudflare Pages가 GitHub Actions로 `mobile_app/`를 배포한다.
...
- 무료 우선 아키텍처: **GitHub Actions + Cloudflare Pages/Workers + Cloudflare D1 동기화 코드**.
- 종합분석은 GitHub Actions로 주기 실행하고, 보유 종목 가격은 Cloudflare Pages Function에서 보수적으로 5분 캐시해 조회한다.
...
- **내 보유 종목**: `window.HOLDING_UNIVERSE`에 분석 종목의 티커/회사명/업종/가격을 심고, `/api/quotes`에서 5분 캐시 가격을 받아 평가손익을 갱신. 동기화 코드는 `/api/sync-code`, `/api/holdings/{code}`가 Cloudflare D1에 저장. Yahoo v7 quote endpoint가 막힐 수 있어 `yahoo-chart` fallback을 사용한다.
...
## 8. 배포 절차 (Cloudflare Pages, 현재 운영)

1. 로컬 수동 빌드 시 `python export_mobile_app.py`로 `mobile_app/` 최신화.
2. 현재 운영 배포는 Cloudflare Pages: `.github/workflows/cloudflare-pages.yml`이 `mobile_app/`를 배포.
3. GitHub secrets `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_D1_DATABASE_ID`가 설정되어 있다.
   워크플로가 배포 직전에 D1 binding `DB`가 포함된 `wrangler.toml`을 임시 생성한다.
   Cloudflare dashboard에서 직접 binding을 붙여도 되지만, secret 기반 설정을 우선 유지.
4. 최신 검증 기준: `https://sweetproduct.pages.dev/` 200, `/api/health` 정상, `/api/quotes?symbols=AAPL,MSFT` 가격 반환, `/api/sync-code` D1 저장 정상.
5. Netlify 수동 배포도 가능하지만, 그 경우 Cloudflare API가 같은 origin에 없으면 동기화 기능은 별도 Worker 라우팅이 필요하다. 기본 운영 대상으로 되돌리지 말 것.
...
- [x] Cloudflare 실제 배포/D1 생성/마이그레이션/GitHub secrets/Actions 배포 연결 완료.
- [ ] 진짜 실시간 시세는 유료 API 가능성이 높으므로 사용자 승인 전 도입 금지. 현재는 무료 `yahoo-chart` fallback + 5분 캐시.
- 테스트: GitHub Actions `pytest -q` 통과. 최신 Cloudflare deploy 성공.
